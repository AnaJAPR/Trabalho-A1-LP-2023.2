from limpa_dados import remove_colunas_sem_dado, trata_celulas_vazias, corrige_nomes_df
import pandas as pd
import matplotlib.pyplot as plt

# definindo o df com as alterações do limpa_dados
def define_df_limpo():
    csv_file = "IGC_2021.csv"
    df = pd.read_csv(csv_file)
    df = remove_colunas_sem_dado(df)
    df = trata_celulas_vazias(df)
    df = corrige_nomes_df(df)
    return df

print(define_df_limpo())
